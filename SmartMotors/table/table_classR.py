...
class SmartMotorTable:
    ...
    def move_selection(self, direction):
        ...
        if 0 <= new_selected_index < len(self.rows):
            ...
            self.last_update_time = time.ticks_ms()  # Reset the timer when selection changes
            self.draw_table()

    def update_left_value(self):
        """Automatically increases the left value (L) every second while selected."""
        ...
        while True:
            if self.view_mode == "table":
                ...
                if pot_position != self.previous_pot_value:
                    ...
                    self.last_update_time = time.ticks_ms()  # Reset timer on selection change
                    self.draw_table()

                # Automatically update L value every second
                ...
                while self.view_mode == "selected_color":
                    if self.button_select.value() == 0:
                        ...
                        self.view_mode = "table"
                        self.previous_pot_value = -1  # Reset potentiometer tracking
                        self.draw_table()
                        break  # Exit cleanly and return to the main loop
